/** This program can be used to run pytorch trt engines with the C++ API
 * 
 ...
 * 
 * root@ines-nano-1:/app# ./torch_trt 
 * -------Running TensorRT Pytorch C++-------
 ...
 * Mean               = 0.0439192 (s)
 * Median             = 0.04 (s)
 * Standard Deviation = 0.00777288
 * Classification Results:
 * 
 * Top:1 index:3 value:8.83386
 * Top:2 index:2 value:5.9161
 * Top:3 index:4 value:3.73487
 * Top:4 index:149 value:3.01505
 * Top:5 index:405 value:2.87074
 * 
 * :Note: imagenet index to label can be found in data/imagenet_idx.json. Tiger shark has index 3.
 *        pytorch model output is not softmaxed by default
...
bool Classifier::init(std::string error_path, int input_size)
{
    ...
    mInputIdx = trt_engine->getBindingIndex("input");
    m_output_idx = trt_engine->getBindingIndex("output");

    // init cuda memory for input                            3 channels (rgb)
    cudaMalloc(&trt_buffer[0], m_input_size * m_input_size * 3 * sizeof(float));

    // init cuda memory for output
...
cv::Mat Classifier::infer(std::string input_path)
{
    ...
    // scale by 255, subtract imagenet mean and divide by imagenet std
    input_image.convertTo(input_image, CV_32FC3, 1 / 255.0);
    cv::subtract(input_image, cv::Scalar(0.485f, 0.456f, 0.406f), input_image);
    cv::divide(input_image, cv::Scalar(0.229f, 0.224f, 0.225f), input_image);

    // convert HWC to CHW and put to gpu see:
    // https://github.com/spmallick/learnopencv/blob/master/PyTorch-ONNX-TensorRT-CPP/trt_sample.cpp
...
int main(int argc, char *argv[])
{
    ...
    // init variables
    std::string model_path("torch.engine");
    ...
    // print some info
    std::cout << "-------Running TensorRT Pytorch C++-------" << std::endl;
